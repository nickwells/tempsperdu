...
func TestIsLeapYear(t *testing.T) {
	...
	for _, tc := range testCases {
		if tempsperdu.IsLeapYear(tc.dt) != tc.exp {
			t.Log(tc.IDStr())
			if tc.exp {
...
func TestDaysInMonth(t *testing.T) {
	...
	for _, tc := range testCases {
		if days := tempsperdu.DaysInMonth(tc.dt); days != tc.exp {
			t.Log(tc.IDStr())
...
func TestDaysInYear(t *testing.T) {
	...
	for _, tc := range testCases {
		if days := tempsperdu.DaysInYear(tc.dt); days != tc.exp {
			t.Log(tc.IDStr())
			t.Errorf("\t: %s should have %d days but has %d\n",
